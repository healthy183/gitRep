 ...
 * Date: 2013-05-21
 * Last Modify Date: 2013-09-27 by Felix @ PCCW for textarea new line enter
 */
//jsHint options
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("th",thr).each(function ( j ) {
			...
			/* Add the required class, when editrules is required (Set font-color blue) - Begin */
			if(ts.p.colModel[j].editrules && ts.p.colModel[j].editrules.required)
                $(">div",this).addClass('required');
			/* Add the required class, when editrules is required (Set font-color blue) - End */
				...
				if (ind){
					...
					if(nData !== "" || forceupd === true) {
						...
						if($t.p.treeGrid && $(".tree-wrap",$(tcell)).length>0) {
							...
							if ($t.p.colModel[pos].edittype && $t.p.colModel[pos].edittype=='textarea') // edited by Felix @20130927 (textarea celledit enter)
                                $(tcell).text(v).attr(title);											// edited by Felix @20130927 (textarea celledit enter)
                            else																		// edited by Felix @20130927 (textarea celledit enter)
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
                        ...
                        /* Add the required class, when editrules is required (Set font-color blue) - Begin */
                        if(this.editrules && this.editrules.required)
                            trdata.addClass('required');
                        /* Add the required class, when editrules is required (Set font-color blue) - End */
		...
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && (e.target.tagName === undefined || e.target.tagName != 'TEXTAREA')) {// edited by Felix @20130927 (textarea celledit enter)
